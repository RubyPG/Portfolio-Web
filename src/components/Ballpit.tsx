...
class X {
  ...
  clear() {
    this.scene.traverse((obj: Object3D<Object3DEventMap>) => {
...
class Z extends InstancedMesh {
  ...
  hemiLight: HemisphereLight | undefined;
  light: PointLight | undefined;
  accentLight: PointLight | undefined;

  constructor(params: Partial<typeof XConfig> = {}) {
    const config = { ...XConfig, ...params };
    const geometry = new SphereGeometry();
    const material = new MeshPhysicalMaterial({ ...config.materialParams });
  ...
  #setupLights() {
    ...
    this.hemiLight = new HemisphereLight(0x3b82f6, 0x0f172a, 0.66);
    this.add(this.hemiLight);
    this.light = new PointLight(0x3b82f6, this.config.lightIntensity * 1.05, 22, 2);
    this.light.position.set(-4, 2.3, 10);
    this.add(this.light);
    this.accentLight = new PointLight(0x1d4ed8, this.config.lightIntensity * 0.55, 20, 2);
    this.accentLight.position.set(4.8, -2.2, 8.2);
    this.add(this.accentLight);
  }
...
function createBallpit(canvas: HTMLCanvasElement, config: any = {}): CreateBallpitReturn {
  ...
  let spheres!: Z;
  threeInstance.minPixelRatio = 1;
  threeInstance.maxPixelRatio = 1.5;
  threeInstance.renderer.toneMapping = ACESFilmicToneMapping;
  threeInstance.renderer.toneMappingExposure = 1.1;
...
const Ballpit: React.FC<BallpitProps> = ({ className = '', followCursor = true, ...props }) => {
  ...
  useEffect(() => {
    ...
    const initBallpit = () => {
      ...
      if (!canvas || !parent) {
        frameId = requestAnimationFrame(initBallpit);
        return;
      }
    ...
    if (canvas?.parentElement) {
      resizeObserver = new ResizeObserver(() => {
        if (spheresInstanceRef.current) {
          ...
          spheresInstanceRef.current.three.render();
          return;
        }
  ...
  return (
    <div className={`relative block w-full h-full overflow-hidden ${className}`}>
